...
`mockModel`. `EVE_DIRECT_OPENAI=1` instead wires the OpenAI SDK
`gpt-5.6-luna` model; it wins over a mock setting and has no scenario-model
fallback. `EVE_MODEL` remains the gateway selection when direct mode is absent.
...
- Eve wrappers still use fixture-backed local refund data. That deliberate lab
  boundary is independent of whether the language model is mocked or direct.
- A completed Eve session is saved to the default `SqliteTraceStore` before its
  report is rendered, making it available to `traces:mine` and
  `assurance:report <run-id>`.

## Decision 12: Deterministic assurance ledger before AI explanation

**Chosen:** Console and JSON reports are deterministic. The optional Markdown
report renders deterministic verdict, claim/evidence checklist, controls,
trajectory, and residual-risk sections first; only the final, explicitly
non-authoritative narrative is generated by an LLM.

**Why:** An explanation can make review faster but must not become a second
source of truth or alter the assurance disposition. Placing verifier and
evidence references before prose lets a reviewer identify unsupported narrative
directly.
...
- Direct Luna wiring has local coverage, but live Luna behavior has not been
  exercised here. Historical `gpt-5.4-mini` results are not a Luna claim.
- A successful keyless Eve eval can still emit local `world-local` queue retry
  warnings after scenario completion; investigate command-level failures rather
  than treating those post-success warnings as model evidence.
- The site adapter clears `docs/` before writing generated output. Preserve and
  restore the tracked technical Markdown and plans when rebuilding the site.
